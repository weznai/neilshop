...
<template>
  <section class="section">
    <div class="container" style="max-width:960px">
      ...
      <div class="meta-row">
        <span>{{ updatedLabel(tt) }}</span><span class="meta-dot" /><span>{{ tt('30-day window · free exchanges, always', '30 天退货窗口 · 换货永久免费') }}</span>
      ...
      <div class="policy-grid" style="display:grid;grid-template-columns:200px 1fr;gap:32px">
        ...
        <article class="prose">
          <div class="grid grid-3 grid-m-1" style="margin-bottom:22px">
            ...
            <div class="card" style="padding:16px;text-align:center"><b style="font-size:20px">{{ tt('3–5 days', '3–5 天') }}</b><div style="font-size:12.5px;color:var(--gray)">{{ tt('Refund speed', '退款时效') }}</div></div>
          </div>
          <h2 id="returns" style="scroll-margin-top:84px">{{ tt('Returns', '退货') }}</h2>
          ...
          <h2 id="exchanges" style="scroll-margin-top:84px">{{ tt('Exchanges (the magic part)', '换货（最贴心的部分）') }}</h2>
          ...
          <h2 id="exceptions" style="scroll-margin-top:84px">{{ tt('Exceptions', '例外情形') }}</h2>
