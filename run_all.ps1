...
if (-not (Test-Path -LiteralPath $Py)) {
    # venv 缺失时回落 PATH 上的 python（依赖已装即可跑），避免无 venv 机器直接 FATAL
    $fallback = Get-Command python -ErrorAction SilentlyContinue
    if ($fallback) {
        $Py = $fallback.Source
        Write-Output "[precheck] venv missing -> fallback python : $Py"
    } else {
        Write-Output "[FATAL] venv python not found: $Py (and no python on PATH)"
        exit 1
    }
} else {
...
$Entries = @()
foreach ($n in @('test_a', 'test_b', 'test_admin_ext', 'test_admin_ops_ext', 'test_admin_flow_ext', 'test_admin_queues_ext', 'test_c', 'test_ai_ext', 'test_chat_ext', 'test_worker', 'test_worker_ext', 'test_refsub', 'test_payments',
'test_obs', 'test_sec', 'test_sec_ext', 'test_rbac', 'test_perf', 'test_perf_ext', 'test_p0', 'test_p0b', 'test_exchanges',
