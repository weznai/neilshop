...
def list_discounts(
    ...
    q: str | None = None,
    admin: User = Depends(require_perm("promo:manage")),
...
def create_discount(body: DiscountCreateIn, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.create_discount(db, admin, body)


@router.put("/api/admin/ops/discounts/{discount_id}")
def update_discount(discount_id: int, body: DiscountUpdateIn, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.update_discount(db, admin, discount_id, body)


@router.post("/api/admin/ops/discounts/{discount_id}/toggle")
def toggle_discount(discount_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
...
def create_giftcard(body: GiftcardAdminCreateIn, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.create_giftcard(db, admin, body)


@router.put("/api/admin/promo/giftcards/{gift_card_id}/freeze")
def freeze_giftcard(gift_card_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.freeze_giftcard(db, admin, gift_card_id)


@router.put("/api/admin/promo/giftcards/{gift_card_id}/unfreeze")
def unfreeze_giftcard(gift_card_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.unfreeze_giftcard(db, admin, gift_card_id)


@router.put("/api/admin/promo/giftcards/{gift_card_id}/void")
def void_giftcard(gift_card_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
...
def delete_discount(discount_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.delete_discount(db, admin, discount_id)


@router.get("/api/admin/ops/popups")
def list_popups(admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.list_popups(db)


@router.post("/api/admin/ops/popups")
def create_popup(body: PopupCreateIn, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.create_popup(db, admin, body)


@router.put("/api/admin/ops/popups/{popup_id}")
def update_popup(popup_id: int, body: PopupUpdateIn, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.update_popup(db, admin, popup_id, body)


@router.post("/api/admin/ops/popups/{popup_id}/toggle")
def toggle_popup(popup_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.toggle_popup(db, admin, popup_id)


@router.delete("/api/admin/ops/popups/{popup_id}")
def delete_popup(popup_id: int, admin: User = Depends(require_perm("promo:manage")), db: Session = Depends(get_db)):
    return service.delete_popup(db, admin, popup_id)


@router.get("/api/admin/ops/settings")
def list_settings(admin: User = Depends(require_perm("settings:manage")), db: Session = Depends(get_db)):
    return service.list_settings(db)


@router.put("/api/admin/ops/settings")
def upsert_setting(body: SettingIn, admin: User = Depends(require_perm("settings:manage")), db: Session = Depends(get_db)):
    return service.upsert_setting(db, admin, body)


@router.get("/api/admin/ops/email-templates")
def list_email_templates(admin: User = Depends(require_perm("settings:manage"))):
